...
- `transcript_abundance` - this tool takes all transcriptome alignments from a set of reads generated by `minimap2 -p0` in [PAF](https://github.com/lh3/miniasm/blob/master/PAF.md) format and estimates transcript abundances using the EM algorithm. This tool is a re-implementation of a [script](https://github.com/jts/nanopore-rna-analysis/blob/master/nanopore_transcript_abundance.py) written by [Jared Simpson](https://oicr.on.ca/investigators/jared-simpson).
...
```
Usage of ./transcript_abundance:
  -V    Print out version.
  -a float
        Minimum aligned fraction for the best hit. (default 0.5)
  -c string
        Compatibility file.
  -f int
        Maximum distance from start when classifying as full length. (default 20)
  -h    Print out help message.
  -m int
        Minimum read length.
  -n int
  ...
  -s float
        Score threshold used when considering equivalent hits. (default 0.95)
  -t int
        Maximum number of cores to use. (default 4)
  -v    Be verbose.
```

This tool requires as input a PAF file generated by `minimap2` with using the `-p0` option. 

Example run:

```bash
minimap2 -t 8 -x map-ont -p0 transcriptome.fas reads.fq | transcript_abundance -v - > abundances.tsv 
```
